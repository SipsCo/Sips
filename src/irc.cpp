...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #sipscoinTEST\r");
            Send(hSocket, "WHO #sipscoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #sipscoin\n");
            // randomly join #sipscoin00-#sipscoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #sipscoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #sipscoin%02d\r", channel_number).c_str());
        };
