...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSipsAmount"))
        settings.setValue("nAnonymizeSipsAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeSipsAmount = settings.value("nAnonymizeSipsAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeSipsAmount"))
        SoftSetArg("-anonymizeSipsamount", settings.value("nAnonymizeSipsAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeSipsAmount:
            nAnonymizeSipsAmount = value.toInt();
            settings.setValue("nAnonymizeSipsAmount", nAnonymizeSipsAmount);
            emit anonymizeSipsAmountChanged(nAnonymizeSipsAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
